...
function exec(func, thisArg, args) {
  return promises.all(args).then(function(args) {
...
function lift(normalFunction) {
  ...
  var invoke = function() {
    ...
    if (!resolvedArgs) {
      return promises.all(args).then(function(resolved) {
        args = resolved;
        resolvedArgs = true;
    ...
    return promises.all(additionalArgs).then(function(additionalArgs) {
